...
function M:Show()
	...
	self.data = {}
	self.data.id = BusinessData.GetOpenID(Const.BAGROUP_RANKRECHARGE)
...
function M:ResetData()
	...
	--排行
	local rank, rankInfo = BusinessData.GetMyRank(self.data.id)
	self:UpdateRank(self.textMyRank, self.imageMyRank, rank)
	self.textDesc.text = BusinessData.GetBAComonRankDesc(self.data.id, "再充<color=#00bb00>%s</color>元宝可升上第<color=#00bb00>%s</color>名")
...
function M:UpdateItem(index, coms)
	...
	if index < 4 and rankCfg and rankCfg.tab.min then
		...
		coms.textDesc.text = string.format("<color=#%s>最低充值元宝%s</color>", color, rankCfg.tab.min)
	else
		coms.textDesc.text = ""
	end
