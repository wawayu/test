...
function M:Show()
	...
	if not self.businessTab or not self.script then
		self:Hide()
		return
	end
...
function M:ResetData()
	...
	self.chargeGoldTotal = (info and info.intparam) or 0
	self.textCharge.text = string.format("当前已累积:%s", self.chargeGoldTotal)
...
function M:UpdateItem(index, coms)
	...
	coms.txtName.text = string.format("累充%s元宝", data.gold)
	coms.textProgress.text = ""
	-- 刷新奖励
